...
- [ ] **Caché de Peaks en Disco (`.peaks`):** Generar archivos binarios ligeros con los picos de onda pre-calculados. Se almacenarán en una **carpeta de caché única y dedicada** (no junto a los archivos de música, para no ensuciar las carpetas del usuario). La primera vez que se use esta función, el programa le preguntará al usuario dónde quiere ubicar su carpeta de caché, con la **ubicación recomendada por defecto en la raíz de donde está instalado el programa** (ej. `C:\LF Automatizador v1.0\cache\`). Esta carpeta será visible y accesible para el usuario. La primera vez que se abra un track se calcula y guarda el `.peaks`; las siguientes se lee directo del disco sin decodificar, reduciendo el tiempo de apertura del editor de segundos a milisegundos. **Acción (Fase 1):** Worker Thread genera y guarda el `.peaks` en la carpeta de caché. **Acción (Fase 2):** Rust lo genera como parte del análisis automático.
- [ ] **Preview en Tiempo Real en Editores de Transición/Pisador:** Actualmente si mueves la pista B mientras suena el audio, este no se actualiza — hay que detener y volver a dar Play. Esto ocurre porque los `BufferSource` nodes son fire-and-forget. **Acción (corto plazo):** Implementar re-seek automático al soltar el drag (stop+play transparente). **Acción (largo plazo):** Usar `AudioWorklet` o delegar el playback de preview al motor Rust, que puede ajustar offsets de mezcla en tiempo real sin reconstruir nodos.
