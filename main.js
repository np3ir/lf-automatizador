...
let uiPrefs = { menuVisible: false, controlsPos: 'top', temp: true, hum: true, leftPanel: true, ext: false, sysLog: true, showRemainingTime: false, cartwall: false };
...
async function fetchArtistMetadataOnline(displayName) {
    ...
    try {
        ...
        const adbResponse = await fetch(adbUrl, { headers: { 'User-Agent': 'LF_Automatizador/1.0' } });
        if (adbResponse.ok) {
    ...
    if (!result.photoUrl && sourceConfig.deezer.enabled) {
        try {
            ...
            const deezerResponse = await fetch(deezerUrl, { headers: { 'User-Agent': 'LF_Automatizador/1.0' } });
    ...
    if ((!result.country && !result.countryCode) && sourceConfig.musixmatch.enabled && sourceConfig.musixmatch.apiKey) {
        try {
            ...
            const mxResponse = await fetch(mxUrl, { headers: { 'User-Agent': 'LF_Automatizador/1.0' } });
            if (mxResponse.ok) {
    ...
    if (!result.photoUrl) {
        for (const wikiLang of ['es', 'en']) {
            ...
            try {
                ...
                const wikiSearchResponse = await fetch(wikiSearchUrl, { headers: { 'User-Agent': 'LF_Automatizador/1.0' } });
                if (wikiSearchResponse.ok) {
                    ...
                    if (page?.title) {
                        ...
                        const summaryResponse = await fetch(summaryUrl, { headers: { 'User-Agent': 'LF_Automatizador/1.0' } });
    ...
    if (data.source === 'internet') {
        try {
            ...
            try {
                ...
                const mbResponse = await fetch(mbUrl, { headers: { 'User-Agent': 'LF_Automatizador/1.0' } });
                if (mbResponse.ok) {
                    const mbData = await mbResponse.json();
    ...
    const template = [
        ...
        },
        {
            ...
            submenu: [
                ...
                { label: 'ℹ️ Acerca de LF Automatizador', click: () => { dialog.showMessageBox(mainWindow, { type: 'info', title: 'Acerca de', message: 'LF Automatizador v1.3' }); }}
            ]
        }
    ];
